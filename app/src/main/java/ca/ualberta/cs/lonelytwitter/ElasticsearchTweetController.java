...
public class ElasticsearchTweetController {
    ...
    public static class GetTweetsTask extends  AsyncTask<String,Void,ArrayList<NormalTweet>> {
        ...
        protected ArrayList<NormalTweet> doInBackground(String... params) {
            ...
            // Hold (eventually) the tweets we get back from elastic search.
            ArrayList<NormalTweet> tweets = new ArrayList<NormalTweet>();

            // NOTE: A HUGE ASSUMPTION IS ABOUT TO BE MADE
            // Assume that only one string is passed in.
            // Not necessary for implementation, we can do it another way for project.


            // the following gets the top "10 000" tweets
            //String search_string = "{\"from\":0,\"size\":10000}";

            // the following searches for the top 10 tweets matching the string passed in (NOTE: assumption mentioned previously )
            String search_string = "{\"query\":{\"match\":{\"message\":\"" + params[0] + "\"}}}";

            // the following gets the top 10000 tweets matching the string passed in (NOTE: assumption mentioned previously )
